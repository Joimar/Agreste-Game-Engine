...
void GameObject::setPosition(glm::vec3 position)
{
	...
	for (int i = 0; i < this->getModel().meshes.size(); i++) {
		//As atribui��es para atualizar os meshes SIMPLESMENTE N�O FUNCIONAM. OS VALORES N�O MUDAM. E AGORA, BOSTIL?! 
		...
		this->getModel().meshes[i].minY += position.y;
		this->getModel().meshes[i].minZ += position.z;
	}
